<script>
    ...
    $: {
        const projection = druid.UMAP.transform(D, 15, 1, 1, 2, "precomputed").to2dArray.map((row) => Array.from(row));
...
<main>
    <VisWrapper title="DR chart" {shown}>
        ...
        <svg {width} {height}>
            {#each Y as [px, py], i}
                <circle cx={scales.x(px)} cy={scales.y(py)} r="4" fill={colorScale(data[i].conference)}>
                    <title>
                        {data[i].title}{"\n"}
